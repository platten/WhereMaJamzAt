"""grooveshark_downloader.py: mp3 file downloader based on the wonderful
                            work of George Stephanos's groove-dl
                            http://github.com/jacktheripper51/groove-dl"""
...
_useragent = "Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/535.11 " \
             "(KHTML, like Gecko) Chrome/17.0.963.56 Safari/535.11"
_referer = "http://grooveshark.com/JSQueue.swf?20120220.01"
wget_template = 'wget --user-agent="%s" --referer=%s ' \
                '--post-data=streamKey=%s -O "%s" "http://%s/stream.php"'
_token = None
...
def getToken():
    ...
    conn.request("POST", "/more.php", json.JSONEncoder().encode(p),
                                    {"User-Agent": _useragent,
                                     "Referer": _referer,
                                     "Content-Type": "",
                                     "Accept-Encoding": "gzip",
                                     "Cookie": "PHPSESSID=" + h["session"]})
    _token = json.JSONDecoder().decode(gzip.GzipFile(
                    fileobj=(StringIO.StringIO(
                        conn.getresponse().read()))).read())["result"]
...
def getResultsFromSearch(query, what="Songs"):
    ...
    conn.request("POST", "/more.php?" + p["method"],
            ...
            {"User-Agent": _useragent,
             ...
             "Accept-Encoding": "gzip",
             "Cookie": "PHPSESSID=" + h["session"]})
    j = json.JSONDecoder().decode(gzip.GzipFile(
                    fileobj=(StringIO.StringIO(
                        conn.getresponse().read()))).read())
    try:
...
def artistGetSongsEx(id, isVerified):
    ...
    conn.request("POST", "/more.php?" + p["method"],
                    ...
                    {"User-Agent": _useragent,
                     ...
                     "Accept-Encoding": "gzip",
                     "Cookie": "PHPSESSID=" + h["session"]})
    return json.JSONDecoder().decode(gzip.GzipFile(
                        fileobj=(StringIO.StringIO(
                            conn.getresponse().read()))).read())
...
def getStreamKeyFromSongIDEx(id):
    ...
    p["header"]["token"] = prepToken("getStreamKeysFromSongIDs",
                                            ":bangersAndMash:")
